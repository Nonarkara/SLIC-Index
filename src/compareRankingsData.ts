...
export const ECHO_CHAMBER_NOTES: Record<string, EchoChamberNote> = {
  ...
  bangkok: {
    slicRank: 66,
    en: "Bangkok does not appear in the Happy City Index top 50, the EIU top 50, or most standard liveability rankings. SLIC places Bangkok at #66 (pure rank) with no public-tier seat — Community 70.7 and Pressure 42.6 still clear the Alpha floor, but every shelf slot was filled by higher-ranked cities first. The gap is the argument. Happy City measures governance delivery and public infrastructure. EIU measures expat-comfort stability. Neither index has a dimension for the world's most visited city (Euromonitor 2024–25), for hospitality and acceptance scores that exceed most peers, for marriage equality (January 2025), for safety at 91.2 out of 100, or for a food, nightlife, and daily-life culture that draws tens of millions of international arrivals per year. SLIC doesn't argue Bangkok is the best-governed city. It argues that governance is one dimension of a liveable city, and Bangkok clears dimensions the other indices omit — without gaming the tier rules to force the anchor city onto the shelf.",
    th: "กรุงเทพฯ ไม่ปรากฏใน 50 อันดับแรกของ Happy City Index, EIU หรือการจัดอันดับความน่าอยู่มาตรฐานส่วนใหญ่ SLIC จัดกรุงเทพฯ ไว้ที่อันดับ #66 (อันดับล้วน) โดยไม่มีชั้นสาธารณะ — Community 70.7 และ Pressure 42.6 ยังผ่านพื้น Alpha แต่ทุกชั้นถูกเติมโดยเมืองที่อันดับสูงกว่าก่อน ช่องว่างนี้คือข้อโต้แย้ง Happy City วัดการส่งมอบธรรมาภิบาลและโครงสร้างพื้นฐาน EIU วัดเสถียรภาพสำหรับผู้ถูกส่งมาทำงาน ไม่มีดัชนีใดมีมิติสำหรับเมืองที่มีนักท่องเที่ยวมากที่สุดในโลก คะแนน Hospitality และการยอมรับที่สูงกว่าเมืองคู่แข่ง ความเท่าเทียมทางการสมรส (มกราคม 2568) ความปลอดภัย 91.2/100 หรืออาหาร ชีวิตกลางคืน และวัฒนธรรมประจำวันที่ดึงดูดนักท่องเที่ยวหลายสิบล้านคนต่อปี",
    zh: "曼谷未能进入 Happy City Index、EIU 或大多数标准宜居排名的前 50 名。SLIC 将曼谷列于第 #66 位（纯分），无公开层级席位 —— 社区 70.7 与压力 42.6 仍越过 Alpha 底线，但所有层级席位已被更高排名城市占满。这一差距本身就是论据。Happy City 衡量治理交付与公共基础设施。EIU 衡量外派稳定性。没有任何指数涵盖：全球访客最多的城市、超越多数同类的接待与接受度分数、婚姻平等（2025 年 1 月）、91.2/100 的安全分，以及每年吸引数千万国际游客的美食、夜生活与日常文化。",
    ko: "방콕은 Happy City Index, EIU 또는 대부분의 표준 생활 가능성 순위 상위 50위에 들지 않습니다. SLIC는 방콕을 순수 순위 #66, 공개 등급 없음으로 평가합니다 — 커뮤니티 70.7과 압력 42.6은 Alpha 최저선을 넘지만 모든 등급 자리가 더 높은 순위의 도시로 먼저 채워졌습니다. 이 격차가 논거입니다. Happy City는 거버넌스 제공과 공공 인프라를 측정합니다. EIU는 주재원 안정성을 측정합니다.",
    ja: "バンコクはHappy City Index、EIU、または標準的な居住性ランキングのトップ50には登場しません。SLICはバンコクを純粋な順位#66、公開ティアなしと位置づけています——コミュニティ70.7とプレッシャー42.6はAlpha最低線をクリアしていますが、すべてのティア席がより高い順位の都市で先に埋まりました。このギャップこそが論拠です。",
  },

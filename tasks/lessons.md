...
## 2026-09-03 · A safety convention that three deploys walked past is not a safety mechanism

- **What went wrong:** The 2026-08-29 fix kept `404.html` in `dist/` and relied on `npm run deploy` running `stage:cf` to strip it before the Cloudflare upload. Three separate deploys since then ran `wrangler pages deploy dist` directly — two by other agents, one by a cut-off session — and each time every deep link on slic.nonarkara.org went back to HTTP 404. The convention was documented in the script, the shim, and a lesson, and it still did not hold, because the failure mode is invisible in a browser: the shim renders the right page and only the status code gives it away.
- **Correct behaviour:** Invert the default so the *safe* state needs no discipline. The shim now lives in `deploy/gh-pages-404.html`, outside `public/`, so a plain `vite build` never emits `dist/404.html`; only `npm run deploy:gh` copies it in. `stage:cf` and `.cf-dist` are gone. Anyone running wrangler by hand from now on ships a correct build.
- **How to recognise:** When the same regression recurs after a documented fix, the fix was procedural and the next fix has to be structural. Ask "what happens if someone ignores the instruction?" — if the answer is "production breaks silently", the instruction is not the fix.
- **Also this session:** two heading skips (h1→h3 on /rankings, h2→h4 on /compare) where the size came from tag-scoped selectors, so tag and selector had to move together; and a rollup chunk-assignment accident that hoisted `publicTierPolicy.js` (17 kB) into the 264 kB `methodology-data` chunk, so the homepage pulled ~118 kB gzip of editorial data for four symbols. Fixed with two `manualChunks` lines; proven by reading the HomePage chunk's static imports, not the Vite log.

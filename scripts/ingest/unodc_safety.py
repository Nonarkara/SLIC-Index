"""
UNODC Intentional Homicide Rate Ingestion — applies country-level homicide data
to all cities in that country as a Tier 3 proxy.

Maps to:
  - personal_safety_raw (viability, weight 5, negative — higher homicide rate = worse)

Source: UNODC via World Bank WDI indicator VC.IHR.PSRC.P5
  "Intentional homicides (per 100,000 people)"

Provider: unodc_wb (Tier 3)
Update frequency: Annual

Usage:
    python3 scripts/ingest/unodc_safety.py [--dry-run]
"""
...
LABEL = "Personal safety (homicide rate)"
PILLAR = "viability"
PROVIDER_ID = "world_bank"
SOURCE_TITLE = "UNODC via World Bank WDI: Intentional homicides (per 100,000 people)"
NOTES_PREFIX = "auto_ingest=unodc_safety"
SCALE = 1.0
STALE_BEFORE = 2018
...
def main() -> int:
    ...
    print("UNODC Intentional Homicide Rate Ingestion")
    ...
    print(f"Fetching {INDICATOR_CODE} → {FIELD}...")
    ...
    for country, cities in country_to_cities.items():
        ...
        for city_row in cities:
            ...
            if existing_provider and existing_provider not in (PROVIDER_ID, ""):
                continue
            ...
            if key in existing_by_key:
                ...
                row["notes"] = f"{NOTES_PREFIX}; indicator={INDICATOR_CODE}; year={year}; country_proxy={country}"
            else:
                ...
                new_row["notes"] = f"{NOTES_PREFIX}; indicator={INDICATOR_CODE}; year={year}; country_proxy={country}"
    ...
    if dry_run:
        print(f"\nDRY RUN — {updates} rows would be updated.")
        return 0

    write_csv(CITY_INPUTS_PATH, existing_rows, CITY_INPUT_FIELDS)
    print(f"\nWrote {updates} values to {CITY_INPUTS_PATH.relative_to(ROOT)}")
    return 0

"""
World Bank Country-to-City Proxy Ingestion — Creative Pillar indicators.

Maps to:
  - innovation_research_intensity_raw (creative, weight 5, positive) ← GB.XPD.RSDV.GD.ZS
  - administrative_investment_friction_raw (creative, weight 4, negative) ← IC.BUS.EASE.XQ (inverted)

Provider: world_bank (Tier 3)
Update frequency: Annual

Usage:
    python3 scripts/ingest/world_bank_creative.py [--dry-run]
"""
...
INDICATORS = [
    {
        ...
        "notes_prefix": "auto_ingest=world_bank_creative",
        "scale": 1.0,
        "offset": 0.0,
        "stale_before": 2018,
    },
    {
        ...
        "label": "Ease of doing business score (inverted to friction)",
        "pillar": "creative",
        "source_title": "World Bank WDI: Ease of doing business score (inverted to friction)",
        "notes_prefix": "auto_ingest=world_bank_creative",
        "scale": -1.0,
        "offset": 100.0,
        "stale_before": 2018,
...
def main() -> int:
    ...
    print("World Bank Creative Pillar Ingestion")
    ...
    for spec in INDICATORS:
        ...
        for country, cities in country_to_cities.items():
            ...
            # Apply transform: stored = offset + scale * raw_value
            # For R&D: stored = 0 + 1.0 * value = value (pass-through)
            # For ease-of-business: stored = 100 + (-1.0) * value = 100 - value (invert)
